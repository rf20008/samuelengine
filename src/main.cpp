...
namespace {
    // How long SamuelEngine gets to think per move.
    ...
    std::unique_ptr<AbstractPlayer> makePlayer(const std::string& colorName) {
        while (true) {
            ...
            switch (std::tolower(static_cast<unsigned char>(choice[0]))) {
                case 'h': return std::make_unique<HumanPlayer>();
                case 'e': return std::make_unique<SamuelEngine>(ENGINE_TIME_LIMIT_SECONDS);
                default:
    ...
    void printResult(GameStatus status) {
        switch (status) {
            case GameStatus::WHITE_WON: std::cout << "Checkmate -- White wins!" << std::endl; break;
            case GameStatus::BLACK_WON: std::cout << "Checkmate -- Black wins!" << std::endl; break;
            case GameStatus::DRAW: std::cout << "Draw." << std::endl; break;
            default: break; // game's still going; nothing to announce
        }
    }
}
