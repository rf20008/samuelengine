...
int SamuelEngine::MoveOrderer::score_move(const Move &mov) {
	...
	if (m_board.move_is_capture(mov)) {
		Piece victim = m_board.getPiece(m_board.get_capturing_square(mov));

		Piece attacker = m_board.getPiece(mov.startingSquare);

		score += 20'000 + 100 * PIECE_VALUES[victim.pieceNum()] - PIECE_VALUES[attacker.pieceNum()];
	}
...
std::pair<int, Move> SamuelEngine::evaluate_chess_pos_with_depth(ChessBoard &board, int depth, int alpha, int beta) {
	...
	// ---- TT STORE ----
	transpositionTable.store(TTEntry{board.getZobrist(), depth, value, flag, bestMove});
