...
bool tryParseMove(ChessBoard &board, const std::string &raw, Move &out) {
	...
	if (s.size() == 4) {
		char fileFrom = static_cast<char>(std::tolower(static_cast<unsigned char>(s[0])));
		char rankFrom = s[1];
		char fileTo = static_cast<char>(std::tolower(static_cast<unsigned char>(s[2])));
		char rankTo = s[3];

		if (fileFrom >= 'a' && fileFrom <= 'h' && fileTo >= 'a' && fileTo <= 'h' && rankFrom >= '1' && rankFrom <= '8' && rankTo >= '1' && rankTo <= '8') {
			...
			for (const Move &move : board.allLegalMoves()) {
				if (move.startingSquare == candidate.startingSquare && move.endingSquare == candidate.endingSquare) {
					out = move;
					return true;
				}
			}
